...
A otimização de modularidade foi realizada empregando o modelo \textit{RBConfigurationVertexPartition}. Para identificar tanto as macrorredes de cooperação ilícita quanto os pequenos anéis fechados de ofuscação, o algoritmo de Leiden foi executado em duas resoluções distintas:
\begin{itemize}
    \item \textbf{Macro-resolução:} Com parâmetro de resolução $\gamma = 1.0$.
    \item \textbf{Micro-resolução:} Com parâmetro de resolução $\gamma = 2.0$.
\end{itemize}
Para cada resolução, extraiu-se o tamanho da comunidade na qual o vértice foi inserido e a pontuação de modularidade local, injetando esses metadados como \textit{features} tubulares do nó correspondente.

\subsection{Padrões de Vizinhança, Agregação e \textit{Motifs}}

Os padrões comportamentais de vizinhança (1-salto) foram extraídos para mapear o grau de estruturação no entorno imediato das entidades financeiras \cite{dong2025spacegnn}.

\begin{itemize}
    \item \textbf{\textit{Egonets} e Coesão Local:} Para cada vértice, gerou-se o seu subgrafo de raio 1 (\textit{ego graph}). A partir dele, calculou-se a densidade local, a contagem de nós periféricos e a soma do peso total circulante (\textit{volume}). Em paralelo, calculou-se o coeficiente de agrupamento local (\textit{local clustering coefficient}) utilizando os pesos financeiros, além da contagem de triângulos baseada na projeção não-direcionada da rede.
    ...
    \item \textbf{Mapeamento de \textit{Motifs} Complexos:} Uma rotina de busca de subgrafos isomorfos foi implementada para contar táticas explícitas de lavagem. O limiar de dispersão e concentração (\textit{fan-out} e \textit{fan-in}) foi estipulado em $k \ge 5$ vizinhos, respeitando um grau máximo de 16 conexões para evitar ruído de contas institucionais gigantes. Com base nessa filtragem, calculou-se as ocorrências encadeadas de \textit{scatter-gather} e \textit{gather-scatter}. Adicionalmente, buscou-se a ocorrência de ciclos simples fechados limitados a até 5 saltos de distância (\textit{length\_bound} = 5), aplicando-se uma trava computacional máxima de 1.000 ciclos processados por nó (\textit{max\_cycles} = 1000).
\end{itemize}

Ao término dessa bateria de extração estrutural, as tabelas tabulares isoladas de cada \textit{chunk} foram concatenadas pelo \texttt{DuckDB}, construindo o \textit{dataset} final enriquecido (\textit{sliding\_window\_features.parquet}) que servirá de matriz de entrada ($X$) para o modelo preditivo.
