...
Devido ao volume massivo de registros gerados pelas variações do \textit{dataset} AMLworld (alcançando até 180 milhões de transações na variação \textit{Large}), o tratamento da base bruta exigiu uma etapa de pré-precessamento e engenharia de dados.
...
O cálculo exato de algumas métricas de centralidade em grafos esparços e de grande escala possui uma complexidade de tempo proibitiva. Portanto, foram adotadas otimizações e parametrizações específicas para o ambiente de \textit{Big Data}:
...
\begin{figure}{\linewidth}
    ...
    \fonte{\citeonline{Elaborado pelo autor}.}
...
Ao injetar este fator diretamente no cálculo do gradiente das árvores de decisão, o algoritmo severamente a penalidade por cada falso negativo, forçando o modelo a mapear as fronteiras de decisão das transações ilícitas sem destruir a densidade informacional da classe majoritária.
...
Este capítulo apresenta os resultados experimentais obtidos através da aplicação da metodologia descrita no Capítulo 3. A avaliação foi conduzida utilizando dois \textit{datasets} sintéticos do AMLworld: o HI Small, contendo 1.245.666 registros transacionais agregados em 8 janelas temporais com taxa de fraude de 1,15\%, e o HI Medium, contendo 7.376.519 registros transacionais com taxa de fraude de 1,35\%. Os experimentos em ambos os \textit{datasets} utilizaram processamento através de janelas temporais deslizantes (\textit{forward-chaining}) para preservar a causalidade estrita.
