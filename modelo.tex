...
\subsubsection{Etapa 1: Proteção por Estabilidade Temporal}

No HI Small, a rotina de poda foi iniciada com 49 \textit{features}. Antes da eliminação recursiva, aplicou-se uma camada de proteção de estabilidade temporal: as \textit{features} no percentil superior de importância SHAP (60\%) foram preservadas para evitar remoções agressivas em atributos estruturalmente críticos ao longo das janelas. Essa etapa protegeu 33 atributos e restringiu a poda aos sinais de menor contribuição preditiva.


\subsubsection{Etapa 2: Eliminação Recursiva Guiada por SHAP}

A partir desse conjunto protegido, aplicou-se a eliminação recursiva, removendo iterativamente as \textit{features} de menor importância SHAP e monitorando a variação de desempenho. A Tabela~\ref{tab:rfe-results} apresenta as primeiras iterações do processo.
...
\begin{table}{\linewidth}
    ...
    \begin{tabular}{ccccl}
        ...
        \hline
        0 & 49 & 0.2932 & -- & (Baseline) \\
        1 & 48 & 0.2935 & +0.0003 & \texttt{fan\_out\_count} \\
        2 & 47 & 0.2920 & -0.0012 & \texttt{fan\_in\_count} \\
        3 & 46 & 0.2900 & -0.0032 & \texttt{cycle\_count} \\
        4 & 45 & 0.2925 & -0.0007 & \texttt{triangle\_count} \\
        5 & 44 & 0.2879 & -0.0053 & \texttt{egonet\_edge\_count} \\
        6 & 43 & 0.2972 & +0.0040 & \texttt{leiden\_micro\_modularity} \textbf{(Ótimo)} \\
        \hline
    \end{tabular}
\end{table}

O processo identificou a iteração 6 (43 \textit{features}) como o ponto ótimo, onde o modelo alcançou AUPRC = 0.2972, com ganho absoluto de +0.0040 em relação ao baseline com 49 \textit{features} (AUPRC = 0.2932). Em termos relativos, esse avanço corresponde a +1,36\%, mantendo estabilidade nas demais métricas globais.

O conjunto final removeu 6 atributos de baixa contribuição (\texttt{fan\_out\_count}, \texttt{fan\_in\_count}, \texttt{cycle\_count}, \texttt{triangle\_count}, \texttt{egonet\_edge\_count}, \texttt{leiden\_micro\_modularity}), preservando 43 \textit{features} com relevância mais robusta ao longo das janelas.

Essa redução de 12,2\% no número de \textit{features} (de 49 para 43), aliada ao ganho de AUPRC, indica que a poda foi conservadora e orientada por estabilidade, priorizando segurança de desempenho em um cenário de forte desbalanceamento de classes.
...
\textbf{TODO --- resultados pendentes nesta rodada.}

Os resultados atualizados de SHAP-RFE para o dataset HI Medium ainda não estavam disponíveis no momento da consolidação deste capítulo. Por esse motivo, todo o conteúdo quantitativo anterior desta subseção foi invalidado deliberadamente para evitar interpretação de números desatualizados.

Assim que a nova execução estiver concluída, esta subseção será preenchida com:
\begin{itemize}
    \item tabela de iterações do RFE (AUPRC, $\Delta$AUPRC e \textit{features} removidas);
    \item análise do ponto ótimo de poda e composição do conjunto final;
    \item gráficos de trajetória de degradação e evolução de métricas.
\end{itemize}
...
A otimização do conjunto de \textit{features} via SHAP-RFE mostrou, para o HI Small, uma poda conservadora de 12,2\% (de 49 para 43 \textit{features}) com ganho de AUPRC (+1,36\%). Para o HI Medium, os resultados de poda desta rodada permanecem pendentes (TODO), e portanto não foram consolidados quantitativamente nesta versão do texto. Mesmo nesse cenário, os achados reforçam a importância de estratégias de seleção de atributos para reduzir custo computacional sem degradar a capacidade de discriminação.
...
\begin{enumerate}
    ...
    \item \textbf{Otimização para implantação operacional:} No HI Small, a poda SHAP-RFE identificou um subconjunto eficiente de 43 \textit{features} (redução de 12,2\%), com ganho de AUPRC em relação ao baseline da própria etapa de poda. No HI Medium, a atualização dessa etapa permanece como tarefa pendente (TODO) nesta versão.
...
\begin{enumerate}
    ...
    \item \textbf{Escala computacional:} Embora a otimização via SHAP-RFE no HI Small tenha reduzido parte do custo de extração de \textit{features}, o processamento de grafos com milhões de nós ainda exige recursos significativos. Além disso, a atualização da poda para o HI Medium permanece pendente (TODO), reforçando a necessidade de estratégias adicionais de eficiência para operação em escala.
