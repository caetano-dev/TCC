...
% Redefinir o ambiente table para o padrão do LaTeX (sem o segundo argumento obrigatório de largura)
\makeatletter
...
\usepackage{hyperref}  % Controla a formação dos hyperlinks
\makeatletter
...
\logo{imagens/logo_uerj_cinza}
\marcadagua{imagens/marcadagua_uerj_cinza}{1}{160}{255}
...
Embora a lavagem de dinheiro movimente entre 2\% e 5\% do PIB mundial, os métodos tradicionais de detecção ainda falham ao analisar contas de forma isolada. Dependentes de heurísticas rígidas, esses sistemas não conseguem mapear as tipologias complexas que se aproveitam da estrutura da rede transacional. Neste trabalho, investigamos como atributos topológicos extraídos de grafos temporais podem mitigar essa lacuna utilizando o algoritmo XGBoost. Para isso, estruturamos um estudo de ablação controlado para comparar um modelo base, treinado com 21 \textit{features} comportamentais, a um modelo completo composto por 49 atributos (as 21 comportamentais somadas a 28 variáveis topológicas). 
Avaliamos a abordagem em dois \textit{datasets} sintéticos do AMLworld: HI Small (1.245.666 transações, 1,15\% de fraude) e HI Medium (7.376.519 transações, 1,35\% de fraude), ambos processados via janelas temporais deslizantes para respeitar a causalidade estrita. A introdução de métricas de rede (como centralidade, detecção de comunidades e padrões de vizinhança) gerou melhorias estatisticamente significativas. No HI Small, observamos aumentos de +18,00\% em AUPRC ($p=0{,}0188$), +46,67\% em precisão ($p=0{,}0070$) e +41,86\% em F1-Score ($p=0{,}0055$). O ganho se manteve robusto no HI Medium, com altas de +18,88\% em AUPRC, +32,28\% em precisão e +29,04\% em F1-Score. 
Por fim, a análise SHAP confirmou o papel decisivo das variáveis de grafo, destacando métricas como HITS, grau de entrada e modularidade de comunidades entre os principais preditores. Os resultados comprovam que o comportamento fraudulento deixa uma assinatura estrutural clara e persistente na rede transacional, mantendo-se consistente independentemente da escala dos dados.
~\\
...
While money laundering circulates between 2\% and 5\% of global GDP, traditional detection systems still struggle to contain it. Relying on rigid heuristics, these systems fail to capture the complex, multi-party typologies that exploit the underlying structure of the transaction network. In this work, we investigate how topological features extracted from temporal graphs can close this gap using the XGBoost algorithm. We designed a controlled ablation study to isolate the impact of network structure, comparing a baseline model trained on 21 behavioral features against an expanded model with 49 attributes (adding 28 topological features).
We evaluated both models on two synthetic datasets from AMLworld: HI Small (1,245,666 transactions, 1.15\% fraud) and HI Medium (7,376,519 transactions, 1.35\% fraud). To respect strict temporal causality, we processed both datasets using sliding time windows. Integrating graph metrics (such as centrality, community detection, and neighborhood patterns) yielded statistically significant performance gains across both benchmarks. On HI Small, the structural features boosted AUPRC by +18.00\% ($p=0.0188$), precision by +46.67\% ($p=0.0070$), and F1-Score by +41.86\% ($p=0.0055$). These gains remained robust on the larger HI Medium dataset, showing improvements of +18.88\% in AUPRC, +32.28\% in precision, and +29.04\% in F1-Score.
Finally, SHAP analysis confirmed the decisive role of graph features, placing metrics like HITS, in-degree, and community modularity among the top predictors. This validates our hypothesis that illicit actors leave distinct, scale-invariant structural signatures on the financial network.
~\\
...
\begin{itemize}
    ...
    \item Implementar um pipeline preditivo baseado em XGBoost com calibração de limiar e pesagem algorítmica dinâmica para mitigar o alto volume de falsos positivos inerente a cenários de detecção de anomalias.
...
\begin{figure}{\linewidth}
    \centering
    \caption{As três fases clássicas da lavagem de dinheiro}
    \vspace{0.2cm}
    \includegraphics[width=0.85\textwidth]{imagens/fases_ml}
    \fonte{\citeonline{eimf_aml}.}
    \label{fig:fases_ml}
\end{figure}
...
Em problemas de classificação binária, cada entidade é rotulada como fraude (classe positiva) ou não fraude (classe negativa) \cite{bishop2006pattern}. A partir das previsões do modelo, obtêm-se quatro resultados possíveis que compõem a matriz de confusão \cite{fawcett2006introduction}:
\begin{itemize}
...
O \textit{Recall} (Sensibilidade) mede a capacidade de recuperar exemplos positivos, indicando a proporção de fraudadores reais que foram detectados:
\begin{equation}
    \mathrm{Recall} = \frac{TP}{TP+FN}
\end{equation}

A \textit{Precision} (Precisão) mede a exatidão dos alertas gerados, ou seja, a proporção de predições de fraude que estavam corretas:
\begin{equation}
...
A Teoria dos Grafos fornece ferramentas matemáticas capazes de modelar interações do mundo real no formato de vértices e arestas \cite{barabasi2016network}. No contexto de transações financeiras, as interações não ocorrem como eventos estáticos e isolados, mas sim como fluxos paralelos que variam no tempo. Por isso, a rede é modelada como um grafo direcionado, ponderado por múltiplos atributos e subdividido em janelas temporais \cite{oliveira2025complex}.
...
A representação de múltiplos pesos nas arestas permite a extração de métricas de centralidade relacionadas à diferentes comportamentos. Neste trabalho, algoritmos como o \textit{PageRank} são calculados em múltiplas formulações independentes, utilizando tanto a matriz de adjacência ponderada pelo volume ($w_{vol}$) quanto a ponderada pela frequência transacional ($w_{count}$). Dessa forma, o modelo identifica não apenas a concentração de grandes volumes de dinheiro, mas também comportamentos de dispersão e estruturação repetitiva (como o \textit{smurfing}), que muitas vezes usa valores menores, mas um número alto de operações \cite{vilella2025weirdnodes}.

\subsection{Grafos Temporais}
A análise estática de um grafo financeiro, a qual agrega todas as transações em uma única topologia atemporal, introduz vazamento de dados (\textit{data leakage}), pois permite que eventos futuros influenciem a extração de \textit{features} de eventos passados. Para preservar a ordem das transações, o sistema deve ser modelado como um grafo temporal estruturado em janelas cronológicas \cite{sizan2025machine}.

Matematicamente, a rede temporal é dividida em uma sequência de grafos instantâneos (ou \textit{snapshots}) $G=\{G_1, G_2, \ldots, G_T\}$. Cada grafo $G_i = (V_i, E_i)$ é construído a partir de uma janela deslizante (\textit{sliding window}) de tamanho fixo $\Delta t$, que avança com um passo temporal definido (\textit{stride}). A extração de propriedades topológicas para avaliar um vértice na janela $i$ restringe-se exclusivamente às arestas $E_i$ observadas dentro desse intervalo específico. Essa delimitação garante que o cálculo das métricas não seja contaminado por transações futuras, utilizando as mesmas restrições informacionais de um ambiente de produção real \cite{sizan2025machine}.

\subsection{Atributos Topológicos}
A topologia local e global da rede fornece sinais preditivos estruturais que não podem ser descobertos quando os vértices são analisados de forma isolada. Os atributos extraídos dividem-se em três categorias principais:
...
\begin{itemize}
\item \textbf{Grau (\textit{Degree}):} Dividido em grau de entrada (\textit{in-degree}) e de saída (\textit{out-degree}), ele mede o número de conexões distintas de um nó. Matematicamente, dado a matriz de adjacência $A$ do grafo, os graus de um vértice $i$ são definidos como:
\begin{equation}
    k_i^{in} = \sum_{j \in V} A_{ji}, \quad k_i^{out} = \sum_{j \in V} A_{ij}
\end{equation}
Contas utilizadas como vértices de passagem ou que possuem uma concentração alta de fundos ilícitos tendem a exibir graus mais altos.

\item \textbf{Algoritmo HITS (\textit{Hyperlink-Induced Topic Search}):} Atribui a cada vértice uma pontuação de \textit{Hub} ($h$) e uma pontuação de \textit{Authority} ($a$) \cite{elkilany2024detecting}. Estas pontuações são atualizadas iterativamente através da relação mutuamente recursiva:
\begin{equation}
    h(u) = \sum_{v \in N_{out}(u)} a(v), \quad a(v) = \sum_{u \in N_{in}(v)} h(u)
\end{equation}
onde $N_{out}(u)$ e $N_{in}(v)$ representam os conjuntos de vizinhos de saída de $u$ e de entrada de $v$, respectivamente \cite{kleinberg1999authoritative}.
...
\end{equation}
Onde $\sigma_{st}$ é o número total de caminhos mais curtos de $s$ para $t$, e $\sigma_{st}(v)$ é o número desses caminhos que passam obrigatoriamente por $v$. Na lavagem de dinheiro, contas de ofuscação atuam como pontes entre diferentes aglomerados da rede \cite{freeman1977set}.
...
\end{equation}
Sendo $N$ o número total de vértices, $B_u$ o conjunto de vértices que apontam para $u$, e $L(v)$ o número de links de saída do vértice $v$. A formulação com $\alpha=0{,}85$ (\textit{deep}) permite ao algoritmo encontrar padrões mais longos  na rede, enquanto $\alpha=0{,}75$ (\textit{shallow}) foca na influência local, otimizando a detecção de anéis de lavagem curtos e rápidos \cite{page1999pagerank}.

\item \textbf{Decomposição \textit{K-Core}:} Avalia a profundidade de aninhamento de uma conta na rede \cite{shin2016corescope, shin2018patterns, conte2024kcores}. Dado um grafo $G = (V, E)$, um subgrafo induzido $H = (V_k, E_k)$ é definido como um \textit{k-core} se for o subgrafo maximal onde o grau de todo vértice $v \in V_k$, restrito a $H$, é maior ou igual a $k$:
\begin{equation}
...
A detecção de comunidades particiona o grafo em subconjuntos de vértices que estão conectados de forma densa internamente e conectados de forma esparsa com o restante da rede \cite{liu2019evaluation}.
\begin{itemize}
\item \textbf{Algoritmo Leiden:} Utilizado pela sua eficiência e garantia de conectividade interna. O algoritmo busca particionar a rede de modo a maximizar a métrica de modularidade ($Q$), proposta originalmente por \citeonline{newman2004finding}, que compara a densidade de arestas dentro das comunidades com a densidade esperada em um grafo aleatório:
\begin{equation}
...
A extração da topologia de entorno e de subgrafos frequentes (\textit{motifs}) captura a mecânica exata das tipologias de fraude \cite{akoglu2010oddball, mitchell2019effectiveness}.
\begin{itemize}
...
Devido ao volume massivo de registros gerados pelas variações do \textit{dataset} AMLworld (alcançando até 180 milhões de transações na variação \textit{Large}), o tratamento da base bruta exigiu uma etapa de pré-processamento e engenharia de dados.
...
Dentro de cada janela de 3 dias, as transações individuais (multiarestas) foram agregadas. É importante ressaltar que os vértices do grafo ($V$) foram modelados para representar \textbf{entidades financeiras} (indivíduos ou empresas) e não contas bancárias isoladas. Essa etapa de mapeamento, realizada com auxílio da tabela de entidades do \textit{dataset} se dá pelo motivo de que criminosos utilizam múltiplas contas para praticar \textit{smurfing}) e simular comportamentos normais. Com a agregação do grafo por entidade, é possível observar o fluxo real de capital, tornando o uso de contas diferentes transparente.
...
Essa etapa de simplificação estrutural é necessária devido ao fato de algoritmos como o \textit{PageRank}, \textit{Betweenness Centrality} e os algoritmos de detecção de comunidades exigirem uma matriz de adjacência simples ponderada, não operando nativamente sobre multigrafos \cite{baigabyl2025using}. Consolidando multiarestas em uma única aresta direcional, é possível preservar a magnitude e a frequência do fluxo financeiro por meio dos atributos de peso, o que torna viável o processamento matemático da rede \cite{vilella2025weirdnodes}.

Durante este processo de agrupamento via banco de dados (SQL e DuckDB), os seguintes atributos consolidados de aresta foram extraídos:
...
O cálculo exato de algumas métricas de centralidade em grafos esparsos e de grande escala possui uma complexidade de tempo proibitiva. Portanto, foram adotadas otimizações e parametrizações específicas para o ambiente de \textit{Big Data}:
...
\begin{itemize}
  \item \textbf{Centralidade de Intermediação (\textit{Betweenness}):} O cálculo exato do \textit{betweenness} requer o cálculo de todos os caminhos mais curtos do grafo, o que é inviável em redes de milhões de nós. Para contornar esta limitação de \textit{hardware}, adotou-se o método de aproximação por amostragem de vértices \cite{riondato2016fast}. A métrica foi calculada extraindo-se caminhos a partir de uma amostra de $k=80$ nós sentinelas (\textit{seed} = 42), garantindo um equilíbrio entre tempo de processamento e fidelidade estrutural.
    ...
    \item \textbf{Decomposição \textit{K-Core}:} Devido à exigência topológica do algoritmo, o grafo direcionado ponderado foi primeiramente convertido para uma projeção simples não-direcionada e sem auto-laços (\textit{self-loops}) \cite{shin2018patterns}. A partir dessa projeção, extraiu-se o valor de \textit{coreness} máximo atingido por cada entidade na referida janela temporal.
\end{itemize}

\subsection{Detecção de Comunidades (Leiden)}

Embora o \texttt{NetworkX} possua ampla gama de algoritmos de grafos, ele apresenta gargalos de performance para detecção de comunidades. Por isso, o grafo foi convertido para a estrutura da biblioteca \texttt{igraph}, permitindo a aplicação da biblioteca \texttt{leidenalg} \cite{traag2019leiden}.
...
\begin{itemize}
    \item \textbf{Macro-resolução ($\gamma = 1{,}0$):} Representa a formulação de modularidade padrão. O algoritmo agrupa vértices de forma mais permissiva, identificando grandes agrupamentos e a estrutura global da rede. No contexto de prevenção à fraude, esta escala mapeia grandes redes de cooperação, revelando se a entidade pertence a um ecossistema grande.
    
    \item \textbf{Micro-resolução ($\gamma = 2{,}0$):} Aumentar o parâmetro de resolução penaliza a união de vértices. Isso faz com que o algoritmo extraia comunidades menores, porém com conectividade interna alta. Para o domínio de \textit{AML}, essa escala é importante para isolar anéis fechados de ofuscação, detectar \textit{smurfing} e mapear empresas de fachada que transferem recursos intensamente entre si em curtos períodos de tempo.
\end{itemize}

Para cada resolução, extraiu-se o tamanho da comunidade (\textit{size}) na qual o vértice foi inserido e a pontuação de modularidade local. Assim, esses dados foram adicionados à tabela de \textit{features} da entidade correspondente. Isso permite compreender se um usuário atua como parte de um grupo criminoso grande ou pequeno.

\subsection{Padrões de Vizinhança, Agregação e \textit{Motifs}}

Os padrões comportamentais de vizinhança foram extraídos para mapear a estrutura no entorno das entidades financeiras \cite{dong2025spacegnn}.
...
\begin{itemize}
    ...
    \item \textbf{Mapeamento de Padrões Topológicos:} Uma função de busca de subgrafos isomorfos foi implementada para quantificar padrões conhecidos de lavagem de dinheiro presentes no \textit{dataset}. O limiar para detecção de estruturas de dispersão e concentração (\textit{fan-out} e \textit{fan-in}) foi estipulado em $k \ge 6$ vizinhos. Para focar a busca no escopo correto, o grau máximo analisado foi restrito a 16 conexões, uma vez que este foi o limite máximo documentado para os padrões sintéticos de fraude injetados no \textit{dataset} AMLworld \cite{altman2023realistic}, atuando também como uma forma de economizar recursos computacionais. Com base nessa filtragem, calcularam-se as ocorrências de \textit{fan-out} e \textit{fan-in}. Adicionalmente, buscou-se a ocorrência de ciclos simples fechados limitados a até 6 saltos de distância (\textit{cycle\_bound} = 6). Como o problema algorítmico de enumeração de ciclos em grafos direcionados possui alta complexidade computacional, podendo crescer exponencialmente em redes densas, aplicou-se uma trava de segurança limitando a extração a um máximo de 2.000 ciclos por nó (\textit{max\_cycles} = 2000), garantindo a viabilidade do processamento em larga escala.
\end{itemize}

\begin{figure}{\linewidth}
    \centering
    \caption{Fan-in, Fan-out e Ciclo Direcionado em Grafos Financeiros}
    \vspace{0.2cm}
    \includegraphics[width=0.85\textwidth]{imagens/faninfanoutcycle}
    \fonte{Elaborado pelo autor.}
    \label{fig:faninfanoutcycle}
\end{figure}
...
\section{Arquitetura Preditiva e Calibração de Limiar com XGBoost}

O processo de modelagem preditiva foi desenhado para lidar com os dois maiores desafios na detecção de lavagem de dinheiro: desbalanceamento extremo de classes, onde as fraudes representam uma fração minúscula do volume total, muitas vezes inferior a 0,1\%, e a necessidade de minimizar os falsos positivos sem comprometer a descoberta de ilícitos reais. Para tanto, adotou-se o algoritmo \textit{eXtreme Gradient Boosting} (XGBoost)\cite{chen2016xgboost} com calibração de limiar baseada em busca de maximização de precisão sob restrição de cobertura (\textit{Recall-constrained threshold calibration})\cite{shukla2025smote}. O algoritmo busca o limiar que maximiza a precisão garantindo um Recall mínimo de 85\% para a janela de validação.

\subsection{Divisão Cronológica dos Dados (\textit{Forward-Chaining})}

Para garantir a validade estatística do experimento e evitar o vazamento de informações futuras para o passado (\textit{look-ahead bias}), empregou-se a validação cruzada temporal (\textit{forward-chaining cross-validation}), na qual os \textit{snapshots} temporais do grafo foram estritamente ordenados de forma cronológica \cite{sizan2025machine}. 

O modelo foi treinado recursivamente em uma janela histórica de dados e testado exclusivamente nas janelas temporais subsequentes (futuras em relação ao treino). Essa abordagem simula o ambiente de produção de uma instituição financeira, onde o algoritmo de \textit{compliance} não possui qualquer conhecimento sobre as transações que ocorrerão no dia seguinte.

\section{Modelos Preditivos e Avaliação de Desempenho} 

Para garantir que o comportamento real das transações seja preservado, optou-se por não aplicar algoritmos de reamostragem sintética, evitando assim distorções no espaço de dados. Em vez disso, a assimetria do problema é tratada diretamente na função de perda do \textit{Gradient Boosting}. A estrutura experimental compara dois modelos preditivos independentes submetidos à validação temporal \textit{forward-chaining}:
...
\begin{itemize}
    ...
    \item \textbf{Modelo Completo (\textit{Full Model}):} Este classificador ingere o conjunto total de dados enriquecido, unindo as variáveis comportamentais do \textit{Baseline} às \textit{features} topológicas de grafos (como \textit{coreness}, comunidades do algoritmo Leiden, \textit{motifs} direcionais e ciclos). O objetivo é isolar e quantificar o ganho real de informação (LIFT) gerado pela topologia da rede no poder preditivo do sistema.
\end{itemize}

\subsection{Ponderação Dinâmica da Função de Perda}
Para lidar com a assimetria do \textit{dataset}, substituiu-se o uso tradicional de \textit{undersampling} (que descarta dados) pela ponderação da função de perda logística do próprio XGBoost (\texttt{scale\_pos\_weight}). 
...
Ao injetar este fator diretamente no cálculo do gradiente das árvores de decisão, o algoritmo aumenta severamente a penalidade por cada falso negativo, forçando o modelo a mapear as fronteiras de decisão das transações ilícitas sem destruir a densidade informacional da classe majoritária.

O desempenho do sistema foi mensurado através da métrica Área Sob a Curva de Precisão-Recall (\textit{AUPRC}) e do ranqueamento de \textit{Precision@k} (onde $k$ assume valores fixos de capacidade investigativa diária, como 100, 300 e 500 entidades). O uso destas métricas avalia o modelo com base na efetividade dos primeiros alertas gerados perante um alto índice de desbalanceamento, refletindo de forma realista o ganho de tempo dos analistas de risco \cite{karim2024scalable}.

\subsection{Estudo de Ablação}

Para quantificar o impacto da adição das \textit{features} topológicas, implementou-se um estudo de ablação controlado, comparando estatisticamente o desempenho de dois modelos: um modelo de base (\textit{Baseline}) treinado exclusivamente com atributos comportamentais e um modelo completo (\textit{Full Model}) que inclui tanto os atributos comportamentais quanto os topológicos extraídos dos grafos.

O experimento foi conduzido sob validação \textit{forward-chaining} estrita, onde cada modelo foi treinado em janelas históricas e testado em janelas temporais subsequentes. Os dados foram avaliados pelo teste $t$ pareado entre as janelas temporais, com o objetivo de comparar o desempenho das \textit{features} com amostras pareadas \cite{dietterich1998approximate, japkowicz2011evaluating}. Para rejeitar a hipótese nula ($H_0$), foi utilizado o nível de significância padrão $\alpha = 0{,}05$ ($95\%$ de confiança). Resultados com $p$-valor inferior a $\alpha = 0{,}01$ ($99\%$ de confiança) foram sinalizados com asterisco duplo.
...
Este capítulo apresenta os resultados experimentais obtidos através da aplicação da metodologia descrita no Capítulo 2. A avaliação foi conduzida utilizando dois \textit{datasets} sintéticos do AMLworld: o HI Small, contendo 1.245.666 registros transacionais agregados em 8 janelas temporais com taxa de fraude de 1,15\%, e o HI Medium, contendo 7.376.519 registros transacionais com taxa de fraude de 1,35\%. Os experimentos em ambos os \textit{datasets} utilizaram processamento através de janelas temporais deslizantes (\textit{forward-chaining}) para preservar a causalidade estrita.

\section{Desempenho dos Modelos Baseline e Full}

A Tabela~\ref{tab:model-summary-small} apresenta o resumo consolidado de desempenho dos dois modelos avaliados em ambos os \textit{datasets}: o modelo de base (\textit{Baseline}), treinado com 21 atributos puramente comportamentais, e o modelo completo (\textit{Full Model}), treinado com 49 atributos (21 comportamentais + 28 topológicos).
...
\begin{table}
    ...
    \begin{tabular}{lSSSS}
        ...
        \hline
        Acurácia & 0,7622 & 0,8408 & +0,0786 & +10,31{\%} \\
        AUPRC & 0,2513 & 0,2965 & +0,0452 & +18,00{\%} \\
        ROC-AUC & 0,9038 & 0,9309 & +0,0271 & +3,00{\%} \\
        F1-Score & 0,0860 & 0,1220 & +0,0360 & +41,86{\%} \\
        Precisão & 0,0450 & 0,0660 & +0,0210 & +46,67{\%} \\
        Recall & 0,8802 & 0,8570 & -0,0230 & -2,61{\%} \\
        \hline
...
Os resultados no HI Small indicam que a adição das \textit{features} topológicas resultou em melhorias substanciais em múltiplas métricas de desempenho. O ganho mais expressivo ocorreu na precisão (+46,67\%), seguido pelo F1-Score (+41,86\%) e pela AUPRC (+18,00\%). Esses incrementos são particularmente relevantes no contexto de detecção de fraudes, onde a redução de falsos positivos impacta diretamente a carga de trabalho das equipes de \textit{compliance}.

\begin{figure}{\linewidth}
    \centering
    \includegraphics[width=0.95\linewidth]{imagens/HI_Small_ablation_metric_comparison}
...
\begin{table}
    ...
    \begin{tabular}{lcccc}
        ...
        \textbf{Métrica} & \textbf{Baseline} & \textbf{Full Model} & \textbf{Lift} & \textbf{Lift (\%)} \\
        \hline
        Acurácia  & 0,8357 & 0,8803 & +0,0446 & +5,34\%  \\
        AUPRC     & 0,2689 & 0,3197 & +0,0508 & +18,88\% \\
        ROC-AUC   & 0,9155 & 0,9401 & +0,0246 & +2,69\%  \\
        F1-Score  & 0,1229 & 0,1585 & +0,0357 & +29,04\% \\
        Precisão  & 0,0662 & 0,0875 & +0,0214 & +32,28\% \\
        Recall    & 0,8550 & 0,8379 & -0,0171 & -2,00\%  \\ \hline
        P@100     & 0,9800 & 1,0000 & +0,0200 & +2,04\%  \\
        P@200     & 0,9850 & 1,0000 & +0,0150 & +1,52\%  \\
        P@300     & 0,9800 & 1,0000 & +0,0200 & +2,04\%  \\
        P@500     & 0,9840 & 1,0000 & +0,0160 & +1,63\%  \\ \hline
        \hline
    \end{tabular}
\end{table}

Os resultados no HI Medium apresentam um padrão consistente com o observado no HI Small, demonstrando a generalização das contribuições dos atributos topológicos a \textit{datasets} de maior escala. A AUPRC apresenta um ganho (+10,85\%) similar ao HI Small (+18,00\%), validando que o benefício das \textit{features} topológicas é robusto e não depende apenas da escala do \textit{dataset}. A precisão (+29,83\%) também mostra ganho substancial, ainda que ligeiramente inferior ao HI Small, sugerindo um \textit{trade-off} entre complexidade e benefício em \textit{datasets} maiores.

\begin{figure}{\linewidth}
    \centering
    \includegraphics[width=0.95\linewidth]{imagens/HI_Medium_ablation_metric_comparison}
...
A comparação entre HI Small e HI Medium revela características importantes sobre a escalabilidade da abordagem topológica. Ambos os \textit{datasets} apresentam ganhos de AUPRC praticamente idênticos (18,00\% vs 18,88\%), sugerindo que o sinal preditivo capturado pelas \textit{features} topológicas é escala-invariante. Entretanto, o ganho em F1-Score e Precisão é mais pronunciado no HI Small, o que pode estar associado a diferenças nas distribuições de fraude ou na estrutura topológica da rede entre os dois \textit{datasets}.

\subsection{Métrica Precision@k e Eficácia Operacional}

A métrica \textit{Precision@k}, que avalia a eficácia operacional do sistema ao ranquear as entidades de maior risco, manteve-se alta para ambos os modelos em ambos os \textit{datasets}. Com P@100 próximo de 0,99 no HI Small e variando entre 0,98 e 1,00 no HI Medium, o sistema demonstra alta confiabilidade na priorização de alertas.

\begin{figure}{\linewidth}
    \centering
    \includegraphics[width=0.95\linewidth]{imagens/HI_Small_precision_at_k_comparison}
...
\begin{figure}{\linewidth}
    \centering
    \includegraphics[width=0.95\linewidth]{imagens/HI_Medium_precision_at_k_comparison}
    \caption{Comparação de Precision@k entre os modelos Baseline e Full Model (HI Medium)}
    \label{fig:precision-at-k-medium}
\end{figure}

Observa-se uma leve redução no \textit{Recall} geral (HI Small: -3,08\%, HI Medium: -2,00\%), o que indica um \textit{trade-off} esperado: ao refinar a fronteira de decisão para reduzir falsos positivos (aumentando a precisão), o modelo torna-se ligeiramente mais conservador, deixando de capturar uma pequena fração de fraudes. No entanto, essa perda é compensada pelo ganho substancial em precisão e pela redução da sobrecarga investigativa.

\subsection{Análise de Desempenho Temporal no HI Small}

A Tabela~\ref{tab:window-comparison} apresenta a comparação detalhada de desempenho entre os modelos \textit{Baseline} e \textit{Full} ao longo das seis janelas de teste do \textit{dataset} HI Small, permitindo avaliar a consistência temporal dos ganhos observados.
...
\begin{table}
    ...
    \begin{tabular}{cSSSSSS}
        ...
        \textbf{Janela} & {\textbf{AUPRC (B)}} & {\textbf{AUPRC (F)}} & {\textbf{Lift AUPRC}} & {\textbf{P@100 (B)}} & {\textbf{P@100 (F)}} & {\textbf{Lift P@100}} \\
        \hline
        4 & 0,261 & 0,292 & +12,0{\%} & 0,99 & 0,97 & -2,0{\%} \\
        5 & 0,326 & 0,392 & +20,2{\%} & 1,00 & 1,00 & +0,0{\%} \\
        7 & 0,224 & 0,300 & +34,2{\%} & 0,96 & 0,93 & -3,1{\%} \\
        9 & 0,292 & 0,360 & +23,0{\%} & 1,00 & 1,00 & +0,0{\%} \\
        11 & 0,352 & 0,419 & +18,9{\%} & 0,99 & 1,00 & +1,0{\%} \\
        13 & 0,258 & 0,346 & +34,0{\%} & 1,00 & 1,00 & +0,0{\%} \\
        \hline
        {\textbf{Média (12 janelas)}} & \bfseries 0,269 & \bfseries 0,320 & \bfseries +19,0{\%} & \bfseries 0,973 & \bfseries 0,961 & \bfseries -1,2{\%} \\
        \hline
    \end{tabular}%
    }
\end{table}

A análise temporal no HI Medium mostra um padrão robusto: o modelo completo apresentou ganhos positivos de AUPRC nas 12 janelas testadas, com \textit{lifts} variando entre +0,7\% e +34,2\%. A janela 3 apresentou ganho marginal (+0,7\%), enquanto as demais mostraram contribuições mais expressivas. Destaca-se a variabilidade maior em comparação ao HI Small, sugerindo que a escala aumentada do \textit{dataset} captura dinâmicas temporais mais complexas.

Para as métricas de \textit{Precision@100}, observa-se um padrão heterogêneo similar ao HI Small: em algumas janelas, o modelo \textit{Baseline} mantém desempenho ligeiramente superior, o que sugere que para alertas de altíssimo risco, os atributos comportamentais isolados já fornecem sinais preditivos robustos em escalas maiores. No entanto, o ganho geral em AUPRC (+19,0\% na média por janela) demonstra que a vantagem das \textit{features} topológicas emerge quando se expande o horizonte investigativo para alertas de médio risco.

\section{Validação Estatística do Estudo de Ablação}

Para determinar se os ganhos observados são estatisticamente significativos e não resultam de variações aleatórias, foram aplicados testes t pareados sobre as diferenças de desempenho entre os dois modelos ao longo das janelas de teste.

\subsection{Testes Estatísticos no HI Small}

A Tabela~\ref{tab:statistical-tests} apresenta os resultados para o \textit{dataset} HI Small.
...
Os testes confirmam que os ganhos observados em acurácia, precisão, F1-Score, ROC-AUC e AUPRC são estatisticamente significativos ao nível de confiança de 95\% ($p < 0{,}05$). Em particular, as métricas de precisão, F1-Score e ROC-AUC apresentaram significância ao nível de 99\% ($p < 0{,}01$), o que valida a contribuição das \textit{features} topológicas.

A ausência de significância estatística no \textit{Recall} ($p = 0{,}534$) e nas métricas P@100 e P@200 sugere que, embora haja variação numérica, essas diferenças podem ser atribuídas à variabilidade natural das janelas temporais. Entretanto, as métricas P@300 ($p = 0{,}0128$) e P@500 ($p = 0{,}0137$) apresentaram ganhos estatisticamente significativos, reforçando o argumento de que as \textit{features} topológicas contribuem especialmente para a discriminação de entidades em níveis intermediários de risco, onde a separação entre fraudes e operações lícitas torna-se mais difícil.

\subsection{Testes Estatísticos no HI Medium}

A Tabela~\ref{tab:statistical-tests-medium} apresenta os testes de significância para o \textit{dataset} HI Medium, ao longo das 12 janelas de teste.
...
Os testes no HI Medium mostram uma significância estatística ainda maior para acurácia, precisão, F1-Score, ROC-AUC e AUPRC, todos com $p < 0{,}01$. A significância elevada para ROC-AUC ($p < 0{,}0001$), indica que a discriminação geral entre fraudes e operações legítimas beneficia-se consistentemente das \textit{features} topológicas em toda a janela temporal. De forma similar ao HI Small, não há significância estatística em Recall, P@100, P@200 e P@300, sugerindo que os modelos capturam igualmente bem as fraudes de altíssimo risco, diferenciando-se principalmente em médios níveis de risco.

\section{Captura de Entidades Fraudulentas em Nível Operacional}

\subsection{Análise em Nível de Entidades}

Além das métricas agregadas, avaliou-se a eficácia dos modelos no HI Small e  HI Medium em encontrar entidades fraudulentas únicas durante o período de teste. Isso simula a quantidade de criminosos que podem ser identificados pelo modelo, mesmo que os mesmos utilizem mais de uma conta bancária para suas atividades. 

No HI Small, do total de 3.842 entidades ilícitas presentes no conjunto de validação:
...
\begin{figure}{\linewidth}
    \centering
    \includegraphics[width=0.85\linewidth]{imagens/HI_Small_entity_level_recall}
    \caption{Captura de entidades fraudulentas: Baseline vs Full Model (HI Small)}
    \label{fig:entity-recall}
\end{figure}

No HI Medium, do total de 29.781 entidades ilícitas presentes no conjunto de validação:

\begin{itemize}
    \item \textbf{Modelo Baseline:} Detectou 26.877 entidades (90,25\% de cobertura).
    \item \textbf{Modelo Full:} Detectou 26.980 entidades (90,59\% de cobertura).
\end{itemize}

A diferença absoluta é de 103 entidades adicionais (+0,34 pontos percentuais), o que representa uma melhoria relativa de 0,38\% na taxa de captura em relação ao modelo base. Embora a captura incremental de 103 entidades fraudulentas no HI Medium seja maior que a de 29 no HI Small, o dataset apresenta uma melhoria percentual menor comparada ao HI Small.

\begin{figure}{\linewidth}
    \centering
    \includegraphics[width=0.85\linewidth]{imagens/HI_Medium_entity_level_recall}
    \caption{Captura de entidades fraudulentas: Baseline vs Full Model (HI Medium)}
    \label{fig:entity-recall-medium}
\end{figure}

A consistência nos ganhos de AUPRC (+18.88\%) entre HI Small e HI Medium, apesar da diferença de escala de 6x, demonstra que o benefício das \textit{features} topológicas não sofre degradação com aumento do volume de dados. Esse resultado tem implicações importantes para a viabilidade de implantação em ambientes de produção com volumes ainda maiores.

\section{Importância Global das Features via SHAP}

Para interpretar o comportamento do modelo e identificar quais atributos exercem maior influência nas predições, aplicou-se a técnica SHAP (\textit{SHapley Additive exPlanations}) \cite{lundberg2017unified} ao modelo completo. Devido à alta complexidade computacional para calcular permutações de 49 features, a análise SHAP de cada janela foi calculada sobre uma amostragem aleatória controlada de 1.000 instâncias de teste.

\subsection{Análise SHAP no HI Small}

A Tabela~\ref{tab:shap-top15} apresenta as 15 \textit{features} de maior importância global no \textit{dataset} HI Small.
...
A distribuição equilibrada entre \textit{features} comportamentais e topológicas no gráfico SHAP sugere que  ambos os tipos de informação são complementares e necessários para uma detecção eficaz de lavagem de dinheiro.

\begin{figure}{\linewidth}
    \centering
    \includegraphics[width=0.95\linewidth]{imagens/HI_Small_global_shap_beeswarm}
...
A Tabela~\ref{tab:shap-top15-medium} apresenta as 15 \textit{features} de maior importância global no \textit{dataset} HI Medium, permitindo avaliar se a hierarquia de importância é preservada em escala maior.
...
A hierarquia de importância das \textit{features} é consistente entre HI Small e HI Medium, com as mesmas três \textit{features} ocupando os primeiros lugares (\texttt{ach\_count\_sent}, \texttt{ach\_count\_recv}, \texttt{hits\_auth}). Destaca-se que \texttt{in\_degree} mantém a 4ª posição em ambos os \textit{datasets}, reforçando que a centralidade de entrada é um sinal robusto de fraude independentemente da escala. A posição de destaque de atributos topológicos a partir da 3ª feature sugere que a topologia da rede financeira fornece sinais fortes para discriminar fraudes.

\begin{figure}{\linewidth}
    \centering
    \includegraphics[width=0.95\linewidth]{imagens/HI_Medium_global_shap_beeswarm}
    \caption{Distribuição de valores SHAP para as principais features do modelo (HI Medium)}
    \label{fig:shap-beeswarm-medium}
\end{figure}

\section{Métricas Topológicas dos Algoritmos}
\label{sec:analise_critica_topologia}

Foram também geradas métricas de avaliação para cada algoritmo de forma isolada, permitindo uma análise de quais são os mais robustos para discriminar entidades ilícitas em diferentes cenários.
Para evitar uma descrição exaustiva das mais de vinte métricas avaliadas, os algoritmos foram agrupados de acordo com seus perfis de desempenho e comportamentos estruturais no grafo de transações. A Tabela~\ref{tab:resumo_topologico} consolida os principais indicadores dos agrupamentos mais relevantes, servindo como âncora numérica para a interpretação crítica subsequente.

\begin{table}
    \centering
    \caption{Resumo comparativo dos agrupamentos métricos estruturais}
    \label{tab:resumo_topologico}
    \resizebox{\linewidth}{!}{%
    \begin{tabular}{p{2.8cm}p{4.3cm}p{2.0cm}p{2.2cm}p{1.8cm}p{4.6cm}}
        \hline
        \textbf{Perfil coesivo} & \textbf{Métrica representativa} & \textbf{ROC-AUC} & \textbf{Avg. Precision} & \textbf{P@10} & \textbf{Descrição} \\
        \hline
        Rankers globais & \texttt{ACH\_COUNT\_SENT}, \texttt{HITS\_HUB}, \texttt{OUT\_DEGREE} & 0,7422--0,7609 & 0,0636--0,0888 & 100,00\% & Sinal forte de dispersão ativa de saída \\
        Filtros de triagem & \texttt{TX\_COUNT}, \texttt{PR\_COUNT} & 0,6014--0,6782 & 0,0748--0,0938 & 100,00\% & Excelente primeira barreira operacional \\
        Assimetria de papel & \texttt{HITS\_AUTH} & 0,6209 & 0,0174 & 0,00\%--10,00\% & Contas de destino se ocultam no fluxo normal \\
        Baixo sinal & \texttt{BITCOIN\_COUNT\_RECV}, \texttt{TIME\_VARIANCE} & 0,5018--0,6181 & 0,0146--0,0171 & 0,00\%--6,25\% & Candidatas à poda por baixo poder discriminativo \\
        Leiden & Comunidades macroestruturais & $Q$ = 0,6588--0,7898 & -- & -- & Boa partição matemática, mas com diluição fraudulenta \\
        \hline
    \end{tabular}%
    }
\end{table}

Os resultados experimentais revelam uma dicotomia clara entre capacidade de ranqueamento global e precisão em horizontes estritos de decisão. O grupo dos \textit{Rankers Globais}, liderado por \texttt{ACH\_COUNT\_SENT} (ROC-AUC de 0,7422), \texttt{HITS\_HUB} (0,7315 no HI Small e 0,7542 no HI Medium) e \texttt{OUT\_DEGREE} (0,7225 no HI Small e 0,7609 no HI Medium), demonstrou a maior robustez para discriminar entidades lícitas de ilícitas ao longo de toda a curva de operação. Sob a ótica topológica, esse comportamento indica que os esquemas de lavagem de dinheiro são fortemente caracterizados pela dispersão ativa e pela volumetria de saída de transações realizadas por nós intermediários.

Em contrapartida, métricas locais centradas na contagem bruta, como \texttt{TX\_COUNT} (ROC-AUC de 0,6412) e \texttt{PR\_COUNT} (0,6014), embora exibam menor poder de separação global, alcançam Precision@10 de 100,00\%. Do ponto de vista prático de engenharia de risco, esses algoritmos funcionam como filtros de topo de funil, ideais para triagem primária. Eles não resolvem o problema de classificação da base como um todo, mas são extremamente eficazes para concentrar a investigação nas anomalias mais evidentes da rede.

Um dos achados mais ricos da análise crítica reside na assimetria estrutural exposta pelo algoritmo HITS. Enquanto o perfil de \textit{hub} (\texttt{HITS\_HUB}) obteve desempenho expressivo, o perfil de autoridade (\texttt{HITS\_AUTH}) colapsou para uma Average Precision média de 0,0174 e mediana de Precision@10 igual a 0,00\%. Esse contraste sugere que as redes criminosas estruturam-se para tornar visíveis as contas de distribuição e intermediação, ao mesmo tempo em que camuflam as contas de destino final. Em termos legais e operacionais, o sinal da fraude aparece na periferia operacional do fluxo, não no ponto terminal onde o capital é finalmente absorvido.

Métricas de contexto financeiro muito específicas, como \texttt{BITCOIN\_COUNT\_RECV} (ROC-AUC de 0,5018) e \texttt{TIME\_VARIANCE} (0,5883 no HI Small e 0,6181 no HI Medium), apresentaram desempenho similar ao aleatório. Essa proximidade com o acaso, combinada a Average Precision muito baixa, indica baixo valor informacional para a tarefa de ranking. Em um pipeline de produção, esses atributos tendem a aumentar custo computacional sem entregar sinal preditivo proporcional.

\subsection{O Paradoxo Estrutural do Algoritmo de Leiden}
\label{subsec:paradoxo_leiden}

A aplicação do algoritmo de Leiden para detecção de comunidades revelou um paradoxo metodológico relevante para o campo de \textit{Anti-Money Laundering}. Por um lado, as métricas de qualidade são excelentes: o escore médio de modularidade ($Q$) permaneceu alto em todas as janelas observadas, oscilando entre 0,6588 e 0,7898, o que atesta partições matematicamente consistentes e estruturalmente densas.

Por outro lado, Leiden falha como filtro autônomo devido ao efeito de diluição em macrocomunidades. No HI Small, 156.844 de 165.832 nós atribuídos (94,60\%) concentraram-se em comunidades com mais de 500 integrantes, enquanto no HI Medium esse número chegou a 635.056 de 665.127 (95,48\%). Nesses grandes agregados, a taxa de fraude interna converge para a linha de base da rede, chegando a 0,9749\% no HI Small e 0,8673\% no HI Medium. Além disso, as comunidades de maior risco capturam apenas frações marginais da fraude total, o que mostra que uma boa partição matemática não implica utilidade operacional direta.

Conclui-se, portanto, que o Leiden é valioso como descrição da macroestrutura da rede, mas inviável como filtro isolado de detecção. Seu papel é complementar, servindo para contextualizar a arquitetura relacional do grafo, e não para substituir as métricas locais de centralidade e dispersão que efetivamente carregam o sinal antifraude.

\section{Discussão dos Resultados}

Os resultados experimentais confirmam que a incorporação de atributos topológicos extraídos de grafos temporais fornece ganhos estatisticamente significativos na detecção de esquemas de lavagem de dinheiro quando comparada a modelos baseados exclusivamente em \textit{features} comportamentais.
...
A validação temporal do estudo de ablação, realizada através de \textit{forward-chaining} em 6 janelas consecutivas, confere robustez aos resultados. A consistência dos ganhos ao longo de 5 das 6 janelas indica que os padrões topológicos aprendidos pelo modelo generalizam bem para períodos futuros, um requisito importante para a implantação em ambientes de produção.

\begin{figure}{\linewidth}
    \centering
    \includegraphics[width=0.95\linewidth]{imagens/HI_Small_ablation_lift}
...
\begin{enumerate}
    ...
    \item \textbf{Robustez temporal:} A validação via \textit{forward-chaining} em 6 janelas temporais demonstrou consistência dos ganhos ao longo do tempo, indicando que os padrões topológicos aprendidos generalizam para períodos futuros.
\end{enumerate}
...
\begin{itemize}
  \item \textbf{Redução de falsos positivos:} Os ganhos em precisão de +46,67\% no HI Small e +32,28\% no HI Medium implicam em uma redução substancial de alertas falsos, diminuindo a sobrecarga investigativa e permitindo que analistas foquem em casos de alto risco real.
...
\begin{enumerate}
    ...
    \item \textbf{Escala computacional:} O processamento de grafos com milhões de nós exige recursos significativos.
