...
A Tabela~\ref{tab:statistical-tests-medium} apresenta os testes de significância para o \textit{dataset} HI Medium, abrangendo as 12 janelas de teste.
...
\subsection{Análise em Nível de Entidades (HI Medium)}

Além das métricas agregadas, avaliou-se o impacto operacional dos modelos no HI Medium através da contagem de entidades fraudulentas únicas efetivamente capturadas durante o período de teste. Do total de 29.781 entidades ilícitas presentes no conjunto de validação:
...
Embora a diferença absoluta seja de apenas 103 entidades adicionais (+0,34 pontos percentuais), esse ganho incremental representa uma melhoria relativa de 0,38\% na taxa de captura em relação ao modelo base. Em um contexto de \textit{compliance} em larga escala, onde cada entidade fraudulenta não detectada pode resultar em perdas financeiras significativas e penalidades regulatórias, esse incremento possui valor prático mensurável. A captura incremental de 103 entidades fraudulentas no HI Medium, apesar da menor melhoria percentual comparada ao HI Small, reflete a complexidade aumentada da rede transacional em escalas maiores, onde entidades fraudulentas podem empregar táticas mais sofisticadas para evitar detecção.
...
Um dos achados mais ricos da análise crítica reside na assimetria estrutural exposta pelo algoritmo HITS. Enquanto o perfil de \textit{hub} (\texttt{HITS\_HUB}) obteve desempenho expressivo, o perfil de autoridade (\texttt{HITS\_AUTH}) colapsou para uma Average Precision média de 0,0174 e mediana de Precision@10 igual a 0,00\%. Fenomenologicamente, esse contraste sugere que as redes criminosas estruturam-se para tornar visíveis as contas de distribuição e intermediação, ao mesmo tempo em que camuflam as contas de destino final. Em termos legais e operacionais, o sinal da fraude aparece na periferia operacional do fluxo, não no ponto terminal onde o capital é finalmente absorvido.

Métricas de contexto financeiro hiperespecíficas, a exemplo de \texttt{BITCOIN\_COUNT\_RECV} (ROC-AUC de 0,5018) e \texttt{TIME\_VARIANCE} (0,5883 no HI Small e 0,6181 no HI Medium), apresentaram desempenho próximo ao limiar de aleatoriedade. Essa proximidade com o acaso, combinada a Average Precision muito baixa, indica baixo valor informacional para a tarefa de ranking. Em um pipeline de produção, esses atributos tendem a aumentar custo computacional sem entregar sinal preditivo proporcional, o que justifica sua poda em iterações futuras de otimização.
